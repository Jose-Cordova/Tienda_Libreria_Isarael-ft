          ...
          <form @submit.prevent="guardar" class="space-y-6">
            <div class="grid grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div class="col-span-2 space-y-2">
                ...
                <Textarea v-model="formulario.descripcion" rows="3" class="w-full border border-gray-200 rounded-xl p-4 text-sm font-bold focus:border-[#0a3622] outline-none" placeholder="Explique brevemente qué ocurrió con el producto..." />
              </div>
            </div>
...
const guardar = async () => {
  if (!formulario.value.producto_id) {
    mostrarToast('Seleccione el producto dañado.', 'warn');
    return;
  }
  ...
  if (!formulario.value.cantidad || formulario.value.cantidad < 1) {
    mostrarToast('Ingrese una cantidad válida.', 'warn');
    return;
  }
  if (!formulario.value.descripcion) {
    mostrarToast('Por favor ingrese el motivo del daño.', 'warn');
    return;
  }
