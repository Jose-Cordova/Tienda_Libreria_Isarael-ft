...
const abrirModalEditar = (categoria) => {
  ...
  form.value = { id: categoria.id, nombre: categoria.nombre }
  limpiarErrores()
  mostrarModal.value = true
}
...
const guardar = async () => {
  ...
  try {
    if (esEdicion.value) {
      ...
      await categoriaStore.updateCategoria(form.value.id, { nombre: form.value.nombre })
      cerrarModal()
      await cargarCategorias()
      mostrarToast('success', 'Categoría actualizada correctamente.')
    } else {
      // CREACIÓN → Swal modal clásico
      await categoriaStore.createCategoria({ nombre: form.value.nombre })
      cerrarModal()
      await cargarCategorias()
      Swal.fire({
