  ...
  <main class="flex-1 bg-[#f4f7f6] p-6 overflow-y-auto relative">
    ...
    <section class="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-xl shadow-sm mb-6 border border-gray-100">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-lg font-extrabold text-[#0a3622] leading-tight">Reporte de Compras</h1>
    ...
    <section class="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
      <div class="bg-[#c6e5d3] px-6 py-4 border-b border-[#b5d9c5] flex items-center justify-between">
        <h3 class="text-[11px] font-black text-[#0a3622] uppercase tracking-widest">Detalle de Compras</h3>
      ...
      <div v-else-if="compras.length === 0" class="flex flex-col items-center justify-center py-12 px-6 gap-3">
        <div class="p-4 bg-slate-50 rounded-full"><i class="pi pi-shopping-cart text-2xl text-slate-400"></i></div>
        ...
        <table class="w-full text-left">
          <thead>
            <tr class="text-[10px] font-bold uppercase tracking-widest border-b border-gray-100">
              <th class="py-3 px-4 text-[#0a3622]/60">Proveedor</th>
              <th class="py-3 px-4 text-[#0a3622]/60">Fecha</th>
              <th class="py-3 px-4 text-[#0a3622]/60">Estado</th>
              <th class="py-3 px-4 text-right text-[#0a3622]/60">Total</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-50 text-sm">
            <tr v-for="compra in comprasPaginadas" :key="compra.numero_factura" class="hover:bg-gray-50 transition-colors">
              <td class="py-3 px-4 text-gray-700">{{ compra.proveedor }}</td>
              <td class="py-3 px-4 text-gray-500">{{ compra.fecha }}</td>
              <td class="py-3 px-4">
                <span :class="compra.estado === 'REGISTRADA' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'"
                  class="px-2 py-0.5 rounded text-[10px] font-black uppercase">{{ compra.estado }}</span>
              </td>
              <td class="py-3 px-4 text-right font-black text-[#0a3622]">${{ Number(compra.total).toFixed(2) }}</td>
            </tr>
          </tbody>
        </table>
...
const totalPaginas    = computed(() => Math.max(1, Math.ceil(compras.value.length / porPagina)));
const comprasPaginadas = computed(() => compras.value.slice((pagina.value - 1) * porPagina, pagina.value * porPagina));
...
const cargarDatos = async () => {
  ...
  try {
    ...
    const { data } = await api.get('/reportes/compras-datos', { params });
...
const generarPDF = async () => {
  ...
  try {
    const params = {};
    // Solo envía los parámetros si el usuario los seleccionó
    if (filtros.value.estado)       params.estado       = filtros.value.estado;
    if (filtros.value.fecha_inicio) params.fecha_inicio = filtros.value.fecha_inicio;
    if (filtros.value.fecha_fin)    params.fecha_fin    = filtros.value.fecha_fin;

    const response = await api.get('/reportes/compras', { params, responseType: 'blob' });
    const url = URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }));
    window.open(url, '_blank');
    setTimeout(() => URL.revokeObjectURL(url), 5000);
  } catch {
    error.value = 'No se pudo generar el PDF. Revisa los registros del servidor.';
  }
};
