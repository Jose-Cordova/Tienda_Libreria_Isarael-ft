    ...
    <div class="bg-white rounded-xl w-full max-w-2xl shadow-2xl overflow-hidden animate-fade-up border border-gray-100">
      ...
      <div class="p-6" v-if="venta">
        ...
        <div class="grid grid-cols-2 gap-4 text-sm mb-6">
          <div>
            <p class="text-gray-400 font-medium">Fecha y hora</p>
            <p class="font-bold text-gray-700">{{ fechaFormateada }}</p>
          </div>
          <div>
            <p class="text-gray-400 font-medium">Estado</p>
            <span
              :class="venta.estado === 'PAGADA' ? 'bg-green-50 text-green-700 border-green-100' : 'bg-yellow-50 text-yellow-700 border-yellow-100'"
          ...
          <div>
            <p class="text-gray-400 font-medium">Método de pago</p>
            <p class="font-bold text-gray-700">{{ venta.metodo_pago?.nombre || '-' }}</p>
          </div>
          <div>
            <p class="text-gray-400 font-medium">Tipo de cliente</p>
            <p class="font-bold text-gray-700">{{ venta.tipo_cliente }}</p>
          </div>
          <div>
          ...
          <div v-if="venta.credito">
            <p class="text-gray-400 font-medium">Cliente crédito</p>
        ...
        <!-- Tabla de productos -->
        <h3 class="font-extrabold text-gray-600 mb-2 text-sm">Productos</h3>
          ...
          <table class="w-full text-left text-xs">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr v-for="detalle in venta.detalle_ventas" :key="detalle.id">
                <td class="py-2 px-3 font-medium">{{ detalle.producto?.nombre }}</td>
                <td class="py-2 px-3 text-center">{{ detalle.cantidad }}</td>
                <td class="py-2 px-3 text-right">${{ Number(detalle.precio_unitario).toFixed(2) }}</td>
                <td class="py-2 px-3 text-right font-bold">${{ Number(detalle.subtotal).toFixed(2) }}</td>
                <td class="py-2 px-3 text-gray-400">{{ detalle.lote?.codigo_lote || '-' }}</td>
              </tr>
            </tbody>
          </table>
        </div>
